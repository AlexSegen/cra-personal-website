...
const ReposGrid = ({isDark}) => {
    ...
    return (
        <div className={styles.ui__repos_container}>
            <h3 className="mb-4">Open source</h3>

            <div className={styles.ui__repos_grid}>
                {
                    loading ? <div>Cargando...</div> :

                    projects.map(item => (
                        <a key={item.id} className={isDark ? styles.ui__repos_item + ' ' + styles.ui__repos_item_isDark : styles.ui__repos_item } href={item.html_url} target="_blank" rel="noopener noreferrer">
                            <h4>{item.name}</h4>
                            <p>{truncateText(item.description)}</p>
                            <div className={styles.ui__repos_stats}>
                                <div>
